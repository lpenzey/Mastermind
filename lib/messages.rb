module Messages
	...
	def display_previous_guesses(previous_guesses)
		system("clear")
		...
		(previous_guesses.length/3).times do
			puts "#{previous_guesses[guess].join} = #{previous_guesses[red_pins].size} red pins and #{previous_guesses[white_pins].size} white pins"
			guess += 3
			red_pins += 3
			white_pins += 3
		end
